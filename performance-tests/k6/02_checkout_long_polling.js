...
export const options = {
  ...
  thresholds: {
    'http_req_duration': ['p(95)<3000'], // Long polling sengaja menahan koneksi
  },
};
...
export default function (data) {
  ...
  const params = {
    ...
    timeout: '10s', // Jangan putus sebelum long-polling server merespon
  };

  // 1. Tembak Checkout dengan tipe Long-Polling
  const checkoutRes = http.post(`${BASE_URL}/api/v1/checkout/long-polling`, payload, params);
  ...
  const isOk = check(checkoutRes, {
    ...
    'jika 200, status payload adalah PENDING': (r) => {
        if (r.status !== 200) return true; // abaikan jika bukan 200
